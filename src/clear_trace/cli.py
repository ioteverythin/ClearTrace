...
def inspect(cassette_path: str):
    ...
    from clear_trace._types import EventType
    from clear_trace.cassette import load_cassette
...
def diff(old_path: str, new_path: str, detailed: bool, diff_output: str | None):
    ...
    from clear_trace.cassette import load_cassette
    from clear_trace.diff import diff_traces
...
def _write_diff_html(old_trace, new_trace, result, output_path: str):
    ...
    from clear_trace.reporters.html import generate_html_report
...
def debug(cassette_path: str, compare: str | None, tools_only: bool, llm_only: bool):
    ...
    from clear_trace._types import EventType
    from clear_trace.reporters.terminal import run_debugger
...
def costs(directory: str, top: int, sort: str, as_json: bool):
    ...
    from clear_trace.reporters.cost_dashboard import CostDashboard
...
def context(cassette_path: str, visual: bool):
    ...
    try:
        ...
        console.print("[red]Install clear_trace[rag] to use context analysis.[/red]")
        sys.exit(1)
...
def snapshot_record(queries: tuple[str, ...], retriever: str, output: str):
    ...
    try:
        ...
        console.print("[red]Install clear_trace[rag] for snapshot support.[/red]")
        sys.exit(1)
...
def rescore(cassette_path: str, scorer: str, judge_model: str, metrics: str, output: str | None):
    ...
    try:
        if scorer == "ragas":
            from clear_trace.rag.scorers import BaseRAGScorer, RagasScorer
            scorer_obj: BaseRAGScorer = RagasScorer(judge_model=judge_model, metrics=metrics.split(","))
        else:
            from clear_trace.rag.scorers import BaseRAGScorer, DeepEvalScorer
...
def export_rag(cassette_dir: str, fmt: str, output: str):
    ...
    try:
        from clear_trace.rag.export import to_csv, to_deepeval_dataset, to_ragas_dataset
    except ImportError:
        console.print("[red]Install clear_trace[rag] for RAG export.[/red]")
        sys.exit(1)
...
def export_finetune(cassette_dir: str, fmt: str, output: str, system: str | None):
    ...
    try:
        ...
        console.print("[red]Install clear_trace for fine-tune export.[/red]")
        sys.exit(1)
...
def analyze(cassette_dir: str, window: int, top: int, output: str | None, gen_skills: str | None):
    ...
    """
    from clear_trace.analysis import PatternDetector, SkillsGenerator
...
def gap_report(golden_dir: str, agent_dir: str, output: str | None, gen_skills: str | None, as_json: bool):
    ...
    """
    from clear_trace.analysis import GapAnalyzer, SkillsGenerator
    from clear_trace.cassette import load_cassette
...
def pr_diff(pr_url: str, token: str | None, task: bool, files: bool):
    ...
    """
    from clear_trace.github import PRFetcher
