...
from clear_trace.cassette import cassette_path_for_test, load_cassette, save_cassette
...
@pytest.fixture
def clear_traceer() -> type[Replayer]:
    """Provide the Replayer class for manual replay in tests.

    Usage:
        def test_something(clear_traceer):
            with clear_traceer("cassettes/test.yaml") as rep:
...
def trace_snapshot(request: pytest.FixtureRequest):
    ...
    """
    from clear_trace.diff import assert_trace_unchanged
